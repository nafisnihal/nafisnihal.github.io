export const projects = [
  {
    title: "Umar Education",
    description:
      "Developed this educational agency website focused on study abroad services using Next.js with server components and server-side rendering (SSR). The project features a custom-built CMS to manage dynamic content efficiently, providing a seamless experience for students exploring international education opportunities.",
    image: "images/Projects/umar.png",
    imageWebp: "images/Projects/umar.webp",
    tags: ["Next.js", "SSR", "Server Componenets", "Material-UI", "Rest API"],
    liveLink: "https://umar-eg.com/",
    id: 1,
    order: 1,
    mainType: "react",
  },
  {
    title: "EdStructure",
    description:
      "A SaaS-based platform for educational institutions was created, featuring two web apps under the IIIMS brand. This portfolio and informational website showcases the products with detailed features. Frontend optimizations were made during development for smooth user experiences and fast load times.",
    image: "images/Projects/edstructure.jpg",
    imageWebp: "images/Projects/edstructure.webp",
    tags: ["React", "React Query", "SCSS", "Bootstrap", "Rest API"],
    liveLink: "https://edstructure.com/",
    id: 2,
    order: 2,
    mainType: "react",
  },
  {
    title: "UXPro Lab",
    description:
      "UXPro Labs was developed as a service similar to bproperty, with additional features including a dynamic newsfeed similar to matchmaking apps. A CMS was managed, and seamless integration with the backend was handled. Reusable components were maintained to ensure efficient development and scalability.",
    image: "images/Projects/uxprolab.png",
    imageWebp: "images/Projects/uxprolab.webp",
    tags: [
      "TypeScript",
      "React",
      "React Query",
      "Material-UI",
      "Reack Hook Form",
    ],
    liveLink: "https://myuxpro.com/",
    id: 3,
    order: 3,
    mainType: "react",
  },
  {
    title: "Borgo",
    description:
      "Borgo is an Interactive Institution Information and Management System (IIIMS) web application for urban areas. A product of EdStructure. I am working on it from scratch.",
    image: "images/Projects/borgo.jpg",
    imageWebp: "images/Projects/borgo.webp",
    tags: ["TypeScript", "React", "React Query", "Ant Design", "TailwindCSS"],
    liveLink: "https://edstructure.com/borgo",
    id: 4,
    order: 4,
    mainType: "react",
  },
  {
    title: "Unique Regency",
    description:
      "Unique Regency is a web app for hotel booking with various reservation features. Collaborated with team at my current company.",
    image: "images/Projects/unique-regency.png",
    imageWebp: "images/Projects/unique-regency.webp",
    tags: ["React", "Material-UI", "Rest API", "Swiper"],
    liveLink: "https://uniqueregencybd.com/",
    id: 5,
    order: 5,
    mainType: "react",
  },
  {
    title: "Britto",
    description:
      "It is an Interactive Institution Information and Management System (IIIMS) web application for rural areas.  A product of EdStructure. I have started working on the front-end part on developing stage.",
    image: "images/Projects/britto.png",
    imageWebp: "images/Projects/britto.webp",
    tags: ["React", "React Query", "SCSS", "Bootstrap", "Rest API"],
    liveLink: "https://edstructure.com/britto",
    id: 6,
    order: 6,
    mainType: "react",
  },
  {
    title: "Literate",
    description:
      "Literate is Learning Management System (LMS) for evereyone. Getting Data from Rest API.",
    image: "images/Projects/literate.png",
    imageWebp: "images/Projects/literate.webp",
    tags: ["React", "React Bootstrap", "Rest API", "Bootstrap"],
    sourceLink: "https://github.com/nafisnihal/literate-learning-app",
    liveLink: "https://literate-d3e2f.web.app/",
    id: 7,
    order: 7,
    mainType: "react",
  },

  {
    title: "Café",
    description:
      "Reactjs website for a café. It has various features like menu, reservation, contact, etc.",
    image: "images/Projects/cafe.png",
    imageWebp: "images/Projects/cafe.webp",
    tags: ["React", "Material-UI"],
    sourceLink: "https://github.com/nafisnihal/cafe",
    liveLink: "https://coffee-cafe-nn.vercel.app/",
    id: 8,
    order: 8,
    mainType: "react",
  },
  {
    title: "Corbel",
    description:
      "Dashboard for a Mobile Application. The system was built to serve farmers with their needed information and services about medicines and crop disease.",
    image: "images/Projects/corbel.png",
    imageWebp: "images/Projects/corbel.webp",
    tags: ["React", "Material-UI", "Rest API", "React Query"],
    id: 9,
    order: 9,
    mainType: "react",
  },
];

export const TimeLineData = [
  { year: 2024, text: "Became Software Engineer with 1.5 years of experience" },
  { year: 2023, text: "Starting my journey as a Junior Software Engineer" },
  { year: 2022, text: "Just completed my undergraduate degree" },
  { year: 2021, text: "Started learning frontend development" },
  { year: 2019, text: "Exploring programming world as a CS student" },
  { year: 2018, text: "Starting my golden university days" },
];

export const languageSkills = [
  {
    name: "JavaScript",
    level: "90%",
  },
  {
    name: "TypeScript",
    level: "80%",
  },
  {
    name: "ES6",
    level: "70%",
  },
  {
    name: "C#",
    level: "60%",
  },
  {
    name: "SCSS/SASS",
    level: "80%",
  },
];

export const frameworkSkills = [
  {
    name: "React",
    level: "90%",
  },
  {
    name: "Next.js",
    level: "80%",
  },
  {
    name: "Redux",
    level: "70%",
  },
  {
    name: "React Query",
    level: "80%",
  },
  {
    name: "Rest API",
    level: "70%",
  },
  {
    name: "React-Hook-Form",
    level: "70%",
  },
  {
    name: "Material-UI",
    level: "80%",
  },
  {
    name: "Ant Design",
    level: "80%",
  },
  {
    name: "TailwindCSS",
    level: "80%",
  },
  {
    name: "Bootstrap",
    level: "70%",
  },
];

export const toolsSkills = [
  {
    name: "Git",
    level: "80%",
  },
  {
    name: "Postman",
    level: "70%",
  },
  {
    name: "VS Code",
    level: "90%",
  },
];
